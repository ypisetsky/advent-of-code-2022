defmodule Day20 do
  ...
  def solve(suffix \\ "", loops, mult) do
    ...
    datamap = Enum.reduce(1..loops, datamap, fn _, datamap -> Enum.reduce(data, datamap, &move_element(&1, &2, mult)) end)
    ...
    len = Enum.count(data)
    mult * (pos_to_value[clamp(zero_pos + 1000, len)] + pos_to_value[clamp(zero_pos + 2000, len)] + pos_to_value[clamp(zero_pos + 3000, len)])
  end
  ...
  def move_element({element, _} = e, data, mult) do
    ...
    ret = Map.new(data, fn {el, pos} -> cond do
      el == e ->
        {e, new_pos}
      pos in old_pos..new_pos -> {el, pos + inc}
      true -> {el, pos}
    end end)
    ret
  end
